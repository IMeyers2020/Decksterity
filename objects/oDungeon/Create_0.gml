    ...
    for (var xx = 0; xx < _dungeonWidth; xx++) {
        for (var yy = 0; yy < _dungeonHeight; yy++) {
            ...
            if (_cell == CELL_TYPES.ROOM) {
                ...
                tilemap_set(layer_tilemap_get_id(layer_get_id("Tiles")), 2, xx, yy);
            }
			else if (_cell == CELL_TYPES.WALL) {
                tilemap_set(layer_tilemap_get_id(layer_get_id("Tiles")), 3, xx, yy);
            }
			else if (_cell == CELL_TYPES.DOOR_OPEN) {
                tilemap_set(layer_tilemap_get_id(layer_get_id("Tiles")), 4, xx, yy);
            }
			else if (_cell == CELL_TYPES.DOOR_CLOSED) {
                tilemap_set(layer_tilemap_get_id(layer_get_id("Tiles")), 5, xx, yy);
            }
        }
    }
}
...
CreateRoom = function(_x1, _y1, _x2, _y2, startRoom = false, overrideType = pointer_null) {
	...
	if(startRoom) {
		...
		instance_create_layer((_x1 * CELL_SIZE + _x2 * CELL_SIZE) / 2, (_y1 * CELL_SIZE + _y2 * CELL_SIZE) / 2, "Instances", oPlayer)	
		camera_set_view_target(view_camera[0], oPlayer);
		camera_set_view_size(view_camera[0], 100, 100);
...
CreateHallway = function(_x1, _y1, _x2, _y2) {
	...
	if(abs(_x1 - _x2) > abs(_y1 - _y2)) {
		// Horizontal hallway
		// Set doors at beginning and end of hallway
		ds_grid_set_region(dungeon, _x1, _y1, _x1, _y2, CELL_TYPES.DOOR_CLOSED);
		ds_grid_set_region(dungeon, _x2, _y1, _x2, _y2, CELL_TYPES.DOOR_CLOSED);
	} else {
		// Vertical hallway	
		// Set doors at beginning and end of hallway
		ds_grid_set_region(dungeon, _x1, _y1, _x2, _y1, CELL_TYPES.DOOR_CLOSED);
		ds_grid_set_region(dungeon, _x1, _y2, _x2, _y2, CELL_TYPES.DOOR_CLOSED);
	}
}
